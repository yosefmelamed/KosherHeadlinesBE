...
app.get('/api/dates', async function(req, res) {
  try {
    ...
    console.error('/api/dates error:', err);
...
// GET /api/markets — proxies Yahoo Finance with 5 min cache
app.get('/api/markets', async function(req, res) {
  try {
    ...
    console.log('Market data fetched: ' + quotes.length + ' quotes');
    res.json(payload);
  } catch (err) {
...
app.post('/api/fetch', async function(req, res) {
  var body = req.body || {};
  var date = body.date;
  ...
  setImmediate(function() {
    try {
      var args = date ? ' ' + date : '';
      execSync('node ' + path.join(__dirname, '../cron/fetch.js') + args, { stdio: 'inherit' });
    } catch (err) {
...
cron.schedule('0 7 * * *', function() {
  ...
  try {
    execSync('node ' + path.join(__dirname, '../cron/fetch.js'), { stdio: 'inherit' });
  } catch (err) {
...
start().catch(function(err) { console.error('Failed to start:', err); process.exit(1); });